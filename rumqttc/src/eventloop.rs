    ...
    MqttMtdEventLoop<'a, H, E, A, D>
{
    ...
    pub async fn poll(&mut self) -> Result<Event, ConnectionError> {
        if self.network.is_none() {
            let (network, (connack, session_ctx)) = match time::timeout(
                Duration::from_secs(self.network_options.connection_timeout()),
                connect_with_mtd(
                    &self.mqtt_options,
                    &self.mqttmtd_options,
                    self.network_options.clone(),
                ),
            )
            .await
...
>(
    ...
    // send ClientHello over preconnect
    let preconnect = PreConnect::new(PayloadType::ClientHello, handshake_buf.split().freeze());
    network.write(Packet::PreConnect(preconnect)).await?;
    network.flush().await?;
    ...
    // receive ServerHello over preconnect
    let preconnect = match network.read().await? {
        Incoming::PreConnect(preconnect) if preconnect.payload_type == PayloadType::ServerHello => {
            Ok(preconnect)
        }
        packet => Err(ConnectionError::NotConnAck(packet)),
    }?;
    handshaker
        .on_server_hello(&preconnect.payload, ephemeral)
        .map_err(|e| ConnectionError::MqttMtdHandshakeFailed(e))?;

    // receive ServerAuth over preconnect
    match network.read().await? {
        Incoming::PreConnect(preconnect) if preconnect.payload_type == PayloadType::ServerAuth => {
            handshake_buf.extend_from_slice(&preconnect.payload);
            Ok(())
        }
        packet => Err(ConnectionError::NotConnAck(packet)),
    }?;
    ...
    // send ClientAuth over mqtt connect packet
    let mut connect = Connect::new(options.client_id());
    connect.keep_alive = options.keep_alive().as_secs() as u16;
    connect.clean_session = options.clean_session();
    connect.last_will = options.last_will();
    connect.login = Some(Login::new_bytes("mtd", handshake_buf.split().freeze()));
...
fn encode_publish<H: HmacHandler, A: AeadHandler>(
    ...
    let (token, tag) = ctx
        .encode_publish_in_place_with_name(&msg_info, publish.topic.as_str().into(), &mut encrypted)
        .map_err(|_| ConnectionError::MqttMtdFailed)?;
    encrypted.extend_from_slice(tag.as_ref());
    publish.payload = encrypted.freeze();
    publish.topic = str::from_utf8(&[token])
        .map_err(|_| ConnectionError::MqttMtdFailed)?
        .to_owned();
    Ok(())
}
...
fn decode_publish<H: HmacHandler, A: AeadHandler>(
    ...
    let topic = ctx
        .decode_publish_in_place_with_name(&msg_info, publish.topic.as_bytes()[0], &mut decrypted)
        .map_err(|_| ConnectionError::MqttMtdFailed)?;
    decrypted.truncate(publish.payload.len() - TAG_LEN);
    publish.payload = decrypted.freeze();
    publish.topic = str::from_utf8(topic.as_ref())
        .map_err(|_| ConnectionError::MqttMtdFailed)?
        .to_owned();
    Ok(())
}
...
fn encode_subscribe<H: HmacHandler, A: AeadHandler>(
    ...
    subscribe.filters = topics
        ...
        .map(|f| {
            str::from_utf8(&[*f])
                .map_err(|_| ConnectionError::MqttMtdFailed)
                .map(|s| SubscribeFilter::new(s.to_owned(), crate::QoS::AtMostOnce))
        })
